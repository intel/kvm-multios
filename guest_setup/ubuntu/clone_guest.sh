...
function next_available_igpu_vf() {
    ...
    local new_vf=()
    local IFS=$'\n'
    local domains=($(virsh list --all | tail -n +3))
    local i
    for (( i=0; i<${#domains[@]}; i++ )); do
        local domain=$(echo ${domains[$i]} | awk '{ print $2}')
        local state=$(echo ${domains[$i]} | awk '{ print $3}')
        local domain_xml=$(virsh dumpxml $domain)
        # Get the domain sriov vf if defined
        local domain_vf_hex=$(xmllint --xpath "string(//domain/devices/hostdev/source/address[@domain='0x0000' and @bus='0x00' and @slot='0x02']/@function)" - <<<"$domain_xml" )
...
function clone_vm() {
    ...
    if [[ ! -z ${SOURCE_DOMAIN_NAME+x} && ! -z "$SOURCE_DOMAIN_NAME" ]]; then
        ...
        local clone_vf_hex=$(xmllint --xpath "string(//domain/devices/hostdev/source/address[@domain='0x0000' and @bus='0x00' and @slot='0x02']/@function)" - <<<"$clone_xml" )
        local clone_vf_num=$(($clone_vf_hex))
        # Get supported display types and concatenate the different types with _
        # e.g if both spice and vnc are supported, return spice_vnc
        local clone_display=$(xmllint --xpath "//domain/devices/graphics/@type" - <<<"$clone_xml" 2>/dev/null | grep -o '"\([^"]*\)"' | sed 's|"||g'|xargs | sed 's/ /_/g')
        ...
        local xmlfile=$(realpath "$scriptpath/../../platform/$PLATFORM_NAME/libvirt_xml/$SOURCE_XML")
        ...
        local clone_vf_num=$(xmllint --xpath "string(//domain/devices/hostdev/source/address[@domain='0' and @bus='0' and @slot='2']/@function)" - <<<"$clone_xml" )
        # Get supported display types and concatenate the different types with _
        # e.g if both spice and vnc are supported, return spice_vnc
        local clone_display=$(xmllint --xpath "//domain/devices/graphics/@type" - <<<"$clone_xml" 2>/dev/null | grep -o '"\([^"]*\)"' | sed 's|"||g'|xargs | sed 's/ /_/g')
        if [[ $clone_vf_num -gt 0 ]]; then
...
function check_domain() {
    ...
    local status=$(virsh list --all | grep "$domain ")
    ...
    if [[ "$FORCECLEAN" == "1" || "$FORCECLEAN_DOMAIN" == "1" ]]; then
        ...
        local state=$(echo $status | awk '{ print $3}')
...
function update_launch_multios() {
    ...
    local xml_file_path=$(realpath $scriptpath/../../platform/$PLATFORM_NAME/libvirt_xml)
    ...
    local file=$(realpath $scriptpath/../../platform/$PLATFORM_NAME/launch_multios.sh)
    if [ ! -f $file ]; then
...
function show_help() {
    printf "$(basename "${BASH_SOURCE[0]}") [-h] [-s source_domain] [-x source_xml] [-n new_domain] [-p platform] \n"
