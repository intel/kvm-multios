...
function kill_by_pid() {
 ...
 function install_dep() {
    which virt-install > /dev/null || sudo dnf install -y virt-install
    which arepack > /dev/null || sudo dnf install -y p7zip zip atool
    which mkisofs > /dev/null || sudo dnf install -y genisoimage
    which virt-viewer > /dev/null || sudo dnf install -y virt-viewer
    which unzip > /dev/null || sudo dnf install -y unzip
    which xmllint > /dev/null || sudo dnf install -y libxml2
    which yamllint > /dev/null || sudo dnf install -y yamllint
    which cabextract > /dev/null || sudo dnf install -y cabextract
    which xmlstarlet > /dev/null || sudo dnf install -y xmlstarlet
}
...
function copy_ovmf() {
    sudo cp /usr/share/edk2/ovmf/OVMF_VARS.fd /usr/share/OVMF/OVMF_VARS_4M.ms.fd
    sudo cp /usr/share/edk2/ovmf/OVMF_CODE.fd /usr/share/OVMF/OVMF_CODE_4M.ms.fd
}
...
function show_help() {
    ...
    printf "%s [-h] [-p] [--disk-size] [--non-whql-gfx] [--non-whql-gfx-installer] [--force] [--viewer] [--debug] [--dl-fail-exit]\n" "$(basename "${BASH_SOURCE[0]}")"
